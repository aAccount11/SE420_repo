...
void setupSpia(void) {
    ...
    GPIO_SetupPinMux(61, GPIO_MUX_CPU1, 15);  // Set as SPISTEA
    GPIO_SetupPinOptions(61, GPIO_OUTPUT, GPIO_PUSHPULL);  // Make GPIO61 an Output Pin
    GpioDataRegs.GPBSET.bit.GPIO61 = 1;  //Initially Set GPIO61/SS High so SPIRAM is not selected

    GPIO_SetupPinMux(21, GPIO_MUX_CPU1, 0);  // Set as GPIO21 for SPIRAM status LED
    GPIO_SetupPinOptions(21, GPIO_OUTPUT, GPIO_PUSHPULL);
    GpioDataRegs.GPACLEAR.bit.GPIO21 = 1;  // Turn off LED initially
    ...
    GPIO_SetupPinMux(60, GPIO_MUX_CPU1, 15);  //Set GPIO60 pin to SPICLKA
    ...
    GpioCtrlRegs.GPBQSEL2.bit.GPIO60 = 3; // Set I/O pin to asynchronous mode recommended for SPI
...
void saveData(float val1, float val2, float val3, float val4, float val5){
    ...
    if (SpiRAM_transmit_done == 0) {
        ...
        if (SpiRAM_buf_len == 100) {
            ...
            GPIO_SetupPinMux(61, GPIO_MUX_CPU1, 15);  // Set as SPISTEA for DMA
