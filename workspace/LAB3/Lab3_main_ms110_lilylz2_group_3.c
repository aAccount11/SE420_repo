...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer0, 200, 5000); // 5ms
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            UART_printfLine(2,"Ang Vel:%.3f",ang_vel);
            UARTPrint = 0;
        }
...
__interrupt void cpu_timer0_isr(void)
{
    ...
    // oscillation to test motor
    u = ampl * sin( 2 * PI * f * time * .005 );
    

    motor_pos = readEnc1();
    setEPWM7A(motor_pos);
    //setEPWM3A(motor_pos);
    // difference equation to find angular velocity
    ang_vel = (motor_pos - motor_posK_1) / .005; 
    motor_posK_1 = motor_pos;
